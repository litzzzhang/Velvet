...
public:
  ...
  static bool PeriodicUpdate(const string &label, float interval,
    ...
    if (l2t[label] < s_timer->m_elapsedTime) {
      l2t[label] = allowRepetition ? l2t[label] + interval
                                   : s_timer->m_elapsedTime + interval;
      return true;
    }
    return false;
  }

  static auto frameCount() { return s_timer->m_frameCount; }

  static auto physicsFrameCount() { return s_timer->m_physicsFrameCount; }

  static auto elapsedTime() { return s_timer->m_elapsedTime; }

  static auto deltaTime() { return s_timer->m_deltaTime; }

  static auto fixedDeltaTime() { return s_timer->m_fixedDeltaTime; }

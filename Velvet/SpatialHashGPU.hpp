...
public:
  ...
  void Hash(const VtBuffer<glm::vec3> &positions) {
    ...
    params.particleDiameter2 =
        Global::simParams.particleDiameter * Global::simParams.particleDiameter;

    HashObjects(particleHash, particleIndex, cellStart, cellEnd, neighbors,
                positions, initialPositions, params);
  }
...
public: // debug
  ...
  inline int HashCoords(int x, int y, int z) {
    int h =
        (x * 92837111) ^ (y * 689287499) ^ (z * 283923481); // fantasy function
  ...
  void Test(const VtBuffer<glm::vec3> &positions) {
    ...
    for (int i = 0; i < positions.size(); i++) {
      ...
      for (auto val : neighbors_truth[i]) {
        ...
        if (neighbors_gpu[i].count(val) == 0 &&
            dist < errorMargin * m_spacing) {
      ...
      for (auto val : neighbors_gpu[i]) {
        auto dist = glm::length(positions[i] - positions[val]);
        if (neighbors_truth[i].count(val) == 0 &&
            dist < errorMargin * m_spacing) {

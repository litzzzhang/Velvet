...
namespace Velvet {
template <class T> class VtBuffer {
public:
  VtBuffer() {}

  VtBuffer(uint size) { resize(size); }
...
template <class T> class VtRegisteredBuffer {
public:
  VtRegisteredBuffer() {}
...
public:
  ...
  void registerBuffer(GLuint vbo) {
    checkCudaErrors(cudaGraphicsGLRegisterBuffer(
        &m_cudaVboResource, vbo, cudaGraphicsRegisterFlagsNone));

    // map (example 'gl_cuda_interop_pingpong_st' says map and unmap only needs
    // to be done once)
    checkCudaErrors(cudaGraphicsMapResources(1, &m_cudaVboResource, 0));
    checkCudaErrors(cudaGraphicsResourceGetMappedPointer(
        (void **)&m_buffer, &m_numBytes, m_cudaVboResource));
...
template <class T> class VtMergedBuffer {
public:
  VtMergedBuffer() {}
  ...
  void registerNewBuffer(GLuint vbo) {
    ...
    size_t offset =
        m_offsets.empty() ? 0 : m_offsets[last] + m_rbuffers[last]->size();
    m_offsets.push_back(offset);

    // copy from rbuffers to vbuffer
    m_vbuffer.resize(m_vbuffer.size() + rbuf->size());
    cudaMemcpy(m_vbuffer.data() + offset, rbuf->data(),
               rbuf->size() * sizeof(T), cudaMemcpyDefault);
  }
  ...
  void sync() {
    ...
    for (int i = 0; i < m_rbuffers.size(); i++) {
      cudaMemcpy(m_rbuffers[i]->data(), m_vbuffer.data() + m_offsets[i],
                 m_rbuffers[i]->size() * sizeof(T), cudaMemcpyDefault);
    }
  }
